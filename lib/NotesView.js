...
export default class NotesView {
  ...
  handleEvents() {
    ...
    sb.on('Ctrl+Backspace', () => this.deleteNote())
    sb.on('Escape', () => {
  ...
  deleteNote() {
    const filePath = this.noteListView.selected
    log('deleteNote', filePath)
    if (!filePath) return

    const confirm = atom.confirm({
      message: `confirm to delete note at ${filePath}?`,
      buttons: ['ok', 'cancel']
    })
    if (confirm === 0) unlinkSync(filePath)
    else log('deleteNote cancelled')
    setTimeout(() => {
      this.searchBar.focus()
    }, 10);
  }
