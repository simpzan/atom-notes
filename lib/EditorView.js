...
class EditorView extends EventEmitter {
    ...
    highlightSelection() {
        ...
        let from = 0, to = currentMarker.getBufferRange().start.row + 1
        if (previousMarker) {
            from = previousMarker.getBufferRange().start.row + 1
            decoration.setPropertiesForMarker(previousMarker, null)
        }

        log(`select marker ${from} -> ${to}, ${this.current}`)
    ...
    _findNavigate(direction) {
        ...
        log(`findNext ${this.current} -> ${index}, count: ${count-1}`)
